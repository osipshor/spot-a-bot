...
Ключ --conf:
Все они активируются через общий ключ --conf либо при вводе ключей при запуске бота (например, ./A-Bot --conf sell_up 1-2 --conf mpp 5), либо через личного Telegram-бота в Торговых параметрах (поочередно вводя по одной команде --conf sell_up 1-2, затем --conf mpp 5). Активные ключи также видны в Telegram-боте в Торговых настройках. Для удаления ключа без перезапуска бота введите --conf НАЗВАНИЕ_КЛЮЧА в Торговых настройках Telegram-бота.
