...
### Ключи запуска:  
Вводятся при запуске бота (например, ./A-Bot -ts -pd -uo -s если бот на линуксе, если установлен на винде, то создаем ярлык и через свойство в нем прописываем,  например, D:\A-Bot.exe -dlog -ts -pd -s)
* `-s` — Прямой запуск бота в обход главного меню  
* `-ts` — Длинный трейлинг позиций (без триггера стоп-лосса, продажа позиции будет происходить частями trailing_part до тех пор, пока не кончатся монеты в позиции)
...
1) Вводим команды `sudo apt update`, а затем `sudo apt install screen` и вводим Enter или Y, если этого требует терминал
2) Вводим `screen -S <name_screen>` (например `screen -S abot`), попадаем сразу в новый screen и запускаем `./A-Bot`  
3) Запустили бота и можете разрывать соединение с сервером, бот будет работать в фоновом режиме в созданном скрине  
...
5) В случае перезагрузки сервера (reboot или другой апокалипсис), подключаемся к серверу и повторяем описанный цикл со 2-го пункта  
...
* `abot_stats` — Отправлять торговую статистику на сайт статистики 
...
### Ключ --conf:
Все они активируются через общий ключ --conf либо при вводе ключей при запуске бота (например, ./A-Bot --conf sell_up 1-2 --conf mpp 5), либо через личного Telegram-бота в Торговых параметрах (поочередно вводя по одной команде обычным сообщением --conf sell_up 1-2, затем --conf mpp 5). Активные ключи также видны в Telegram-боте в Торговых настройках. Для удаления ключа без перезапуска бота введите --conf НАЗВАНИЕ_КЛЮЧА в Торговых настройках Telegram-бота.

Ключ tov (активируется командой --conf tov ЧИСЛО) — Добавляет определённую сумму к quantity_aver при каждом новом усреднении (от 0.01 до 0.25)
...
Ключ oaq (активируется командой --conf oaq old) — Старая опция из предыдущих версий, добавляющая к новому объёму усреднения allQuantity (общее уже имеющееся количество монет при усреднении) в том случае, если новый объём усреднения меньше minNotional (минимального биржевого ордера, на USDT это 10$ и т.д.). В новых версиях Вы можете поставить quantity_aver, например, 0.01 и бот очень много раз будет усредняться на ~10$, но при активации этого ключа эти ~10$ будут складываться со старым количеством монет в позиции, формируя новый размер усреднения, как allQuantity + minNotional / askPrice вместо minNotional / askPrice в новых версиях
